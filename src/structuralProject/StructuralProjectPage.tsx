...
import {type RJPStudioStructurePackage} from '../engineering/studioExchange'
import {downloadStructureIfc,inspectIfc} from '../engineering/ifcExchange'
...
export default function StructuralProjectPage(){
 ...
 const studioPackage=useMemo<RJPStudioStructurePackage>(()=>({schema:'RJP-STUDIO-STRUCTURE/1.0',source:{app:'SmartStruct RJP',version:'116.1.3',exportedAt:new Date().toISOString()},project,nodes:model.nodes.map(n=>({id:n.id,x:n.x,y:n.y,z:0})),members:model.elements.map(e=>{const a=model.nodes.find(n=>n.id===e.a),bnode=model.nodes.find(n=>n.id===e.b),vertical=a&&bnode?Math.abs(a.x-bnode.x)<1e-9:false,env=envelope?.find(x=>x.id===e.id);return{id:`E${e.id}`,kind:vertical?'column':'beam',a:e.a,b:e.b,geometry:{length:a&&bnode?Math.hypot(bnode.x-a.x,bnode.y-a.y):0,axis:vertical?'Z':'X'},material:{id:`C${fck}`,name:`Betão C${fck}`,family:'Betão',properties:{fck_MPa:fck,fyk_MPa:fyk}},results:env?{N_kN:env.N,V_kN:env.V,M_kNm:env.M,Ncase:env.Ncase,Vcase:env.Vcase,Mcase:env.Mcase}:undefined,reinforcement:steelSchedule.rows.filter(r=>(vertical?r.type==='Pilares':r.type==='Vigas')&&r.element.includes(String(e.id))).map(r=>r.notation).join(' + ')||null}}),foundations:foundations.map(f=>({id:`F${f.id}`,node:f.node,kind:'isolated_footing',geometry:{B_m:f.B,L_m:f.L,h_m:f.h},actions:{N_kN:f.Ry,H_kN:Math.abs(f.Rx),M_kNm:f.M},results:{qmax_kPa:f.r.qmax,utilizacao:f.r.bearingUtil,ok:f.r.ok},reinforcement:steelSchedule.rows.filter(r=>r.type==='Sapatas'&&r.element===`F${f.id}`).map(r=>r.notation).join(' + ')||null})),steelSchedule:steelSchedule.rows,notes:['Pacote de transferência para RJP_3D Studio.','Resultados mantêm o caráter e limitações dos módulos SmartStruct que lhes deram origem.']}),[project,model,envelope,foundations,steelSchedule.rows,fck,fyk])
 const ifcRef=useRef<HTMLInputElement>(null)
 function exportStudio(){saveProject(project);downloadStructureIfc(studioPackage,name)}
 async function importIfc(file?:File){if(!file)return;try{const h=inspectIfc(await file.text());if(h.name)setName(h.name);if(h.location)setLocation(h.location);if(h.floors)setFloors(h.floors);if(h.bays)setBays(h.bays);if(h.bay)setBay(h.bay);if(h.storey)setStorey(h.storey);const total=Object.values(h.counts).reduce((a,b)=>a+b,0);alert(`IFC lido: ${total} elementos reconhecidos. Os parâmetros de projeto SmartStruct encontrados foram aplicados. Geometrias IFC genéricas complexas permanecem para coordenação/openBIM.`)}catch(e){alert(e instanceof Error?e.message:'Não foi possível ler o IFC.')}}
 ...
 {tab==='project'&&<section className="panel"><h3>Projeto</h3><div className="form-grid"><T l="Nome" v={name} s={setName}/><T l="Localização" v={location} s={setLocation}/><T l="Engenheiro" v={engineer} s={setEngineer}/></div><input ref={ifcRef} hidden type="file" accept=".ifc" onChange={e=>importIfc(e.target.files?.[0])}/><div className="editor-toolbar"><button onClick={()=>saveProject(project)}>Guardar localmente</button><button className="primary" onClick={exportStudio}>Exportar IFC · openBIM</button><button onClick={()=>ifcRef.current?.click()}>Importar IFC</button><button onClick={exportJson}>Exportar dados do projeto</button></div><p className="note">IFC4 é o formato de intercâmbio com o RJP 3D Studio e outras plataformas BIM. O SmartStruct exporta geometria estrutural simplificada, materiais, ações/resultados disponíveis e Property Sets RJP; na importação reaproveita parâmetros e entidades suportados sem fingir interpretar geometrias IFC arbitrárias.</p></section>}
